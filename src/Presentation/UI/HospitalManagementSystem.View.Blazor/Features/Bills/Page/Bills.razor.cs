...
public partial class Bills : AuthenticationComponentBase
{
    ...
    #region [ CTor ]
    public Bills(NavigationManager navigator) : base(navigator)
    {
        State = new();
    ...
    private async Task HandleAddDialog(DialogResult result)
    {
        ...
        if (bill is null || bill.SelectedEpisode is null)
        {
            ToastService.ShowToast(ToastIntent.Error, "Not enough information to create a bill");
            return;
        }
    ...
    void HandleTextFilterInput()
        => Refresh(State.CurrentPage, State.LastNameFilter, State.AmountRadioChoice, State.CreatedDateRadioChoice, State.DeadlineRadioChoice);
    ...
    void ClearFormats()
    {
        ...
        Refresh(State.CurrentPage, State.LastNameFilter, "1", "1", "1");
    }
    ...
    #region [ RefreshData ]
    private void Refresh(int index = 1,
